...
app.post('/updatedatabase',async (req,res)=>{
   ...
   try {
    ...
    .from(tableName)
    .upsert(
      [{ email: email, access_token: accessToken, client_id: client_id, refresh_token: accessToken, token_uri: token_uri, client_secret: client_secret, scopes: '', expiry: expiry }],
...
app.get('/mainusers', async (req, res) => {
  try {
    ...
    const { data, error } = await supabase.from('mainusers').select('*');
